...
for i in range(K):
    ...
    # predict abundances
    x, loss = FCLS_unmix(endmembs.spectra, mixture_noisy.spectra)
...
print(metrics.mean())
i = 0
while os.path.exists(result_path + today + "_FCLS_metrics%s.csv" % i):
    i += 1
metrics.to_csv(result_path + today + "_FCLS_metrics%s.csv" % i)
