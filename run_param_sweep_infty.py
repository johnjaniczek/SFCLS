...
from unmix import inftyNorm_unmix
import numpy as np
import pandas as pd
import copy
...
lambdas = np.logspace(-7, -1, 35)  # weights of L1 regularization term
...
for lam in lambdas:
    ...
    for i in range(K):
        ...
        # predict abundances
        x = inftyNorm_unmix(endmembs.spectra, mixture_noisy.spectra, lam=lam)
    ...
    print("lambda: %f, Error_L1_mean: %f" % (lam, Error_L1_mean[-1]))


# store results
results = pd.DataFrame({"lambda": lambdas,
                        "thresh": thresh,
                        "K": K,
                        "SNR": SNR_mean,
...
i = 0
while os.path.exists(result_path + today + "_inftyNorm_sweep_results%s.csv" % i):
    i += 1
results.to_csv(result_path + today + "_inftyNorm_sweep_results%s.csv" % i)
