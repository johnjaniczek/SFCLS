...
lam = 1e-7                           # weight of LASSO regularization
...
print(metrics.mean())
i = 0
while os.path.exists(result_path + today + "_SFCLS_themis_metrics%s.csv" % i):
    i += 1
metrics.to_csv(result_path + today + "_SFCLS_themis_metrics%s.csv" % i)
