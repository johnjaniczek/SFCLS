...
lambdas = np.logspace(-7, -1, 10)  # weights of L1 regularization term
K = 20                        # number of mixtures to test
...
for lam in lambdas:
    ...
    for i in range(K):
        ...
        # predict abundances
        x, loss = SFCLS_unmix(endmembs.spectra, mixture_noisy.spectra, lam=lam)
...
i = 0
while os.path.exists(result_path + today + "_SFCLS_sweep_results%s.csv" % i):
    i += 1
results.to_csv(result_path + today + "_SFCLS_sweep_results%s.csv" % i)
