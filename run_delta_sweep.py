...
delta_range = np.logspace(-5, 0, 10)
lambdas = np.logspace(-10, -1, 10)  # weights of L1 regularization term
K = 50                         # number of mixtures to test
gauss_dev = 2.5e-10              # standard deviation of gaussian noise added to mixture
...
for delta in delta_range:
    for lam in lambdas:
        ...
        for i in range(K):
            ...
            x, loss = delta_norm(endmembs.spectra, mixture_noisy.spectra, lam=lam, delta=delta)
        ...
        print("delta: %f, lambda: %f, Error_L1_mean: %f" % (delta, lam, Error_L1_mean[-1]))
...
i = 0
while os.path.exists(result_path + today + "_delta_sweep_results%s.csv" % i):
    i += 1
results.to_csv(result_path + today + "_delta_sweep_results%s.csv" % i)
