...
lambdas = np.logspace(-7, -1, 70)  # weights of L1 regularization term
...
for lam in lambdas:
    ...
    for i in range(K):
        ...
        # predict abundances
        x = LASSO_unmix(endmembs.spectra, mixture_noisy.spectra, lam=lam)
        x = x / sum(x)
...
i = 0
while os.path.exists(result_path + today + "_LASSO_sweep_results%s.csv" % i):
    i += 1
results.to_csv(result_path + today + "_LASSO_sweep_results%s.csv" % i)
